...
def _sync_update_data_from_base() -> bool:
    ...
    for sheet_name, ws in worksheets.items():
        ...
        logger.info(
            "Лист '%s': %d строк, %d колонок. Реальные заголовки: %s",
            sheet_name, len(data_rows), n_cols, real_headers
        )
...
async def update_data_from_base() -> bool:
    loop = asyncio.get_running_loop()
    result = await loop.run_in_executor(_executor, _sync_update_data_from_base)
    return result
...
def need_update() -> bool:
    """
    Проверяет, нужно ли обновлять базу:
      - если файла update_base_info.json нет → нужно обновить (True)
      - если прошло больше 3 часов с last_updated_utc → нужно обновить (True)
      - иначе → обновлять не нужно (False)
    """
    update_info_file = os.getenv("UPDATE_INFO_FILE", "update_base_info.json")

    if not os.path.exists(update_info_file):
        logger.warning("Файл update_base_info.json не найден — требуется обновление базы.")
        return True
    ...
    try:
        ...
        logger.error("Ошибка чтения update_base_info.json: %s", e)
        return True  # считаем, что нужно обновить

    last_updated_str = info.get("last_updated_utc")
    if not last_updated_str:
        logger.warning("В update_base_info.json отсутствует last_updated_utc — требуется обновление.")
        return True

    try:
        last_updated = datetime.fromisoformat(last_updated_str)
        if last_updated.tzinfo is None:
            # Если нет таймзоны, считаем, что это UTC (как мы пишем)
            last_updated = last_updated.replace(tzinfo=timezone.utc)
    except ValueError as e:
        logger.error("Некорректный формат last_updated_utc: %s", e)
        return True

    now = datetime.now(timezone.utc)
    delta = now - last_updated
    max_age_seconds = 3 * 60 * 60  # 3 часа

    if delta.total_seconds() > max_age_seconds:
        logger.info(
            "База устарела: прошло %.1f часов (лимит 3). Требуется обновление.",
            delta.total_seconds() / 3600,
        )
        return True
    else:
        logger.debug(
            "База актуальна: прошло %.1f часов. Обновление не требуется.",
            delta.total_seconds() / 3600,
        )
        return False
